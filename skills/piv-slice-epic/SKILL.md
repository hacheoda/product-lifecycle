...
### Step 2.5 — Pull the VLAEG components out before slicing

If the architecture doc has a `## VLAEG components` section, those components **do not become PIV
tickets.** They are not sliced, planned, or implemented by this loop — each runs its own Vision →
Link → Architecture → Elegance → Gateway cycle under the `vlaeg-protocol` skill.

What this step produces instead:

- **One entry per VLAEG component in the dependency graph**, marked `[VLAEG]`, carrying only the
  contract the product sees (what it is called with, what it returns, when it runs). Tickets that
  consume it depend on that entry, never on its internals.
- **A thin integration ticket** wherever the product side has real work to do to call it — the
  adapter, the config, the failure path when the component is down or returns stale data. That
  ticket *is* PIV-sized and gets sliced normally.

If there is no such section and you are slicing something that calls an external service on a
schedule, **stop and flag it** instead of slicing it into ordinary tickets. That means the
`plan-architecture` step was skipped, and slicing over it buries the problem in a backlog where
