...
**Input intent**: $ARGUMENTS — a PRD, an epic, a brief, a `spec-draft.md` from the `pm-decision` skill, or a free-form idea. A `spec-draft.md` is the richest of these: its epics are already scored with RICE and its cut line is already drawn, so treat the committed scope as settled intent and do not reopen what to build. Architect **one epic at a time**; the spec's epics are sized to be one architecture pass each — you are here to decide how. If it is a tracker reference (a
...
## Unattended components → route them to VLAEG

Some of what you are architecting is not product code. Apply this test to **each component** you
identify, never to the project as a whole:

1. It calls an external service you do not control.
2. It moves or transforms data between systems.
3. It runs unattended — cron, schedule, webhook, queue consumer.

**All three true → it is a VLAEG component.** Decisive signal: *it can fail at 3am with nobody
noticing.* A product with a nightly sync job is the common case, not the exception, so expect the
answer to be "some of it" rather than yes or no.

For each component that passes:

- **Name it explicitly** under `## VLAEG components` in the output doc. An unmarked one silently
  becomes an ordinary ticket and loses the treatment it needed — that is the whole failure this
  step exists to prevent.
- **Decide its boundary**: what the product calls, what comes back, when it runs. That contract is
  all the product side ever needs to know.
- **Do not design its internals here.** It gets its own Vision → Link → Architecture → Elegance →
  Gateway cycle under the `vlaeg-protocol` skill, which is where the external-connection handshake
  and the three-layer split actually happen.

Where it runs and how the product talks to it (in-process module, separate service, scheduled
worker, queue) *are* architecture calls and belong here. Everything inside the boundary belongs to
VLAEG.
...
## VLAEG components
<one line per component that passes the 3am test: name · what it talks to · trigger · the contract
the product sees. "None" is a valid and common answer — write it rather than deleting the heading,
so the reader knows the test was actually run.>
